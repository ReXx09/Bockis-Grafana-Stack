...
function setProgress(message){document.getElementById('progress').textContent=message;logEvent(message)}
...
document.querySelector('.main-nav').addEventListener('click',event=>{const button=event.target.closest('button[data-view]');if(!button)return;document.querySelectorAll('.main-nav button').forEach(item=>item.classList.toggle('active',item===button));document.querySelectorAll('.view-section').forEach(section=>section.classList.toggle('active',section.id===button.dataset.view));setProgress(`${button.textContent} geoeffnet`)})
...
async function state(){const response=await fetch('/api/state');const data=await response.json();document.getElementById('services').innerHTML=Object.entries(data.services).map(([name,item])=>{const link=SERVICE_LINKS[name]||(DIRECT_PORTS[name]?`http://${location.hostname}:${data.config[DIRECT_PORTS[name]]}/`:'');const endpoint=name==='grafana'?`${location.host}/grafana/`:DIRECT_PORTS[name]?`${location.hostname}:${data.config[DIRECT_PORTS[name]]}`:'&ndash;';const webui=link?`<a href="${link}" target="_blank" rel="noreferrer">Oeffnen</a>`:'&ndash;';const actions=['start','stop','restart','update','logs'].concat(link?['check']:[]).map(action=>`<button data-service="${name}" data-action="${action}">${action==='check'?'WebUI pruefen':action}</button>`).join('');return `<tr><td><strong>${name}</strong></td><td>${webui}</td><td><code>${endpoint}</code></td><td><small class="muted">${item.container} &middot; ${item.image}</small></td><td><span class="badge ${statusClass(item.status)}">${item.status}</span></td><td class="actions">${actions}</td></tr>`}).join('');if(data.docker_error){document.getElementById('message').textContent='Docker-Fehler: '+data.docker_error;logEvent('Docker-Fehler: '+data.docker_error)}else if(data.last_error){document.getElementById('message').textContent='Letzter Installationsfehler: '+data.last_error;logEvent('Installationsfehler: '+data.last_error)}if(!formFilled){for(const [key,value] of Object.entries(data.config)){const field=document.querySelector(`#setup [name="${key}"]`);if(field&&field.type!=='password')field.value=value}formFilled=true}}
document.getElementById('services').addEventListener('click',async event=>{const button=event.target.closest('button[data-action]');if(!button)return;const service=button.dataset.service,action=button.dataset.action;if(action==='check'){setProgress(`${service}: WebUI wird geprueft...`);const response=await fetch(`/api/services/${service}/webui-check`);const data=await response.json();const message=data.error?`${service}: WebUI-Pruefung fehlgeschlagen: ${data.error}`:data.reachable?`${service}: WebUI erreichbar (${data.status} ${data.reason})`:`${service}: WebUI nicht erreichbar: ${data.error||`${data.status} ${data.reason}`}`;document.getElementById('message').textContent=message;setProgress(message);return}if(action==='logs'){setProgress(`${service}: Container-Log wird geladen...`);const response=await fetch(`/api/services/${service}/logs`);const data=await response.json();document.getElementById('logs-title').textContent=`Logs: ${service}`;document.getElementById('logs-content').textContent=data.logs||data.error||'(keine Ausgabe)';document.getElementById('logs-modal').showModal();setProgress(data.error?`${service}: Log-Fehler: ${data.error}`:`${service}: Container-Log geladen`);return}button.disabled=true;setProgress(`${service}: ${action} gestartet...`);const response=await fetch(`/api/services/${service}/${action}`,{method:'POST'});const data=await response.json();button.disabled=false;if(data.error){document.getElementById('message').textContent=data.error;setProgress(`${service}: ${action} fehlgeschlagen: ${data.error}`);return}document.getElementById('message').textContent=action==='update'?(data.update_available?`Neues Image fuer ${service} heruntergeladen - Neustart empfohlen.`:`${service} ist bereits aktuell.`):`${service}: ${action} ausgefuehrt.`;setProgress(document.getElementById('message').textContent);state()});
